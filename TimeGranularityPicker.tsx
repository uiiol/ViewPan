...
function getShortcuts(g: Granularity): Array<{ label: string; range: [string, string] }> {
  ...
  if (g === 'week') return [
    ...
    { label: '近4周', range: [`${now.subtract(3, 'week').year()}-W${String(now.subtract(3, 'week').isoWeek()).padStart(2, '0')}`, `${y}-W${String(w).padStart(2, '0')}`] },
  ]
  // day
  return [
...
const WeekPanel: React.FC<{ sel: string[]; onSelect: (k: string) => void }> = ({ sel, onSelect }) => {
  ...
  const baseWeek = dayjs().add(offset, 'week')
  const w = baseWeek.isoWeek()
  const y = baseWeek.year()
  const k = `${y}-W${String(w).padStart(2, '0')}`
  const start = baseWeek.startOf('isoWeek')
  const end = baseWeek.endOf('isoWeek')
  const label = `${start.format('MM/DD')} — ${end.format('MM/DD')}`
  const isEdge = sel.includes(k)
  const shortcuts = [-4, -3, -2, -1, 1, 2, 3, 4]

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        <Button size="small" icon={<LeftOutlined />} onClick={() => setOffset(o => o - 1)} />
        <div style={{ textAlign: 'center', minWidth: 130 }}>
          <div style={{ fontWeight: 500, fontSize: 13 }}>{y}年 第{w}周</div>
          <div style={{ fontSize: 12, color: '#999' }}>{label}</div>
        </div>
        <Button size="small" icon={<RightOutlined />} onClick={() => setOffset(o => o + 1)} />
      </div>
      <Button
        block
        type={isEdge ? 'primary' : 'default'}
      ...
      <div style={{ fontSize: 12, color: '#999', marginBottom: 6 }}>快速跳转</div>
      <Space wrap>
        {shortcuts.map(n => (
          <Button key={n} size="small" onClick={() => setOffset(o => o + n)}>
            {n < 0 ? `前${Math.abs(n)}周` : `后${n}周`}
          </Button>
        ))}
      </Space>
    </div>
